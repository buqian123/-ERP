      ...
      <view class="content-container">
        ...
        <view v-for="(item, index) in formData.keyMessages.slice(1)" :key="index">
          <view style="position: relative; margin-top: 15px;">
            <image src="/static/del-imp-img.png" class="add-del-imp-info self-del-imp-info" @tap="delImp(index)"></image>
...
export default {
  ...
  methods: {
    ...
    delImp(i) {
      ...
      // let i = (this.formData.keyMessages || []).findIndex(res => res.id === item.id)
      // this.formData.keyMessages.splice(i,1)
    },
    // 选择产品类型
