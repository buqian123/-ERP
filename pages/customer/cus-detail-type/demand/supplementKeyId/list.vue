  ...
  methods: {
    ...
    getDemandInfo() {
      this.$u.api.getDemandInfoById({id: this.demandId}).then(res => {
        for (let i in this.demandInfo) {
          this.demandInfo[i] = res[i]
        }
        this.demandInfo.id = this.demandId
      })
    },
    add() {
      uni.navigateTo({
    ...
    selectKeyInfo(data) {
      ...
      this.demandInfo.keyId = data.keyId
    },
    submit() {  
      this.$u.api.editDemandInfo(this.demandInfo).then(res => {
        uni.redirectTo({
