...
export default {
  ...
  methods: {
    ...
    getSignInfo(id) {
      this.$u.api.getSignList({demandId: id}).then(res => {
        // console.log(res);
        this.signInfo = res
      })
    },
    getEditInfo(id) {
      this.paymentForm.id = id
      this.$u.api.getCollectById({id: id}).then(res => {
        this.getSignInfo(res.demandId)
        for(let i in this.paymentForm) {
          this.paymentForm[i] = res[i]
        }
        this.paymentForm.oldMoney = res.money
        res.imgUrl.forEach(item => {
          let obj = {
            url: item
          }
        ...
        this.settlementMethodList.forEach(item => {
          if (item.value == res.settlementMethod) {
            this.paymentForm.settlementMethod = item.value
            this.settlementMethodText = item.label
          }
        })
    ...
    submit() {
      ...
      this.$u.api.editTransact(this.paymentForm).then(res => {
        // uni.navigateBack({
        //   delta: 1
        // })
        uni.redirectTo({
